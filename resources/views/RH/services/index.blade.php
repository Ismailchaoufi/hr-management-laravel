...
@section('content')
                        ...
                        <tbody>
                            @foreach ($services as $service)
                                <tr>
                                    <td>{{ $service->id }}</td>
                                    <td>{{ $service->nomFr }}</td>
                                    <td>{{ $service->division->nomFr }}</td>
                                    <td>{{ $service->division->nomAr }}</td>
                                    <td>
                                        <a href="#editserviceModal{{ $service->id }}" class="edit" data-toggle="modal">
                                            <i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i>
                                        </a>
                                        <a href="#deleteserviceModal{{ $service->id }}" class="delete" data-toggle="modal">
    ...
    @foreach ($services as $service)
        <div id="editserviceModal{{ $service->id }}" class="modal fade">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('service.update', $service->id) }}" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Nom (FR)</label>
                                <input type="text" name="nomFr" class="form-control" value="{{ $service->nomFr }}" required>
                            </div>
                            <div class="form-group">
                                <label>Nom (AR)</label>
                                <input type="text" name="nomAr" class="form-control" value="{{ $service->nomAr }}" required>
                            </div>
                            <div class="form-group">
                                <label for="id_division">Division</label>
                                <select name="id_division" id="id_division" class="form-control form-select" required>
                                    ...
                                    @foreach ($divisions as $division)
                                        <option value="{{ $division->id }}" 
                                            {{ $service->id_division == $division->id ? 'selected' : '' }}>
                                            {{ $division->nomAr }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    @foreach ($services as $service)
        <div id="deleteserviceModal{{ $service->id }}" class="modal fade">
            <div class="modal-dialog">
                <div class="modal-content">
