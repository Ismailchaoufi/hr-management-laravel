...
class GanttDemande extends Controller
{
    ...
    {
        $users = User::where('role','!=',1)->where('role','!=',2)->with(['demandes' => function ($query) {
    ...
    public function generateGanttChartByDivision(Request $request,$id)
    {
        $division=Division::find($id);
        $users = User::where('id_division', $id)
                    ->where('role', '!=', 1)
                    ->with(['demandes' => function ($query) {
                        $query->where('id_status', 2)
                            ->whereIn('id_typeDemande', [2, 3, 4]);
                    }])->get();
        //for chef division peux voir seulement ses fonctionnaires
        if(auth()->user()->role=2) $users=$users->where('role','!=',2);
        ...
        $writer = IOFactory::createWriter($spreadsheet, 'Xlsx');
        $fileName = 'gantt_chart_division_' . $division->nomFr . now()->format('Y_m_d_H_i_s') . '.xlsx';
        $filePath = storage_path('app/public/' . $fileName);
